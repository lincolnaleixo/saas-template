...
SELECT 'pending' as status, filename 
FROM unnest(ARRAY['0001_init.sql', '0002_users.sql']) as filename
...
```bash
# Automatic rollback happens in transaction
...
bun run scripts/rollback.ts 0002_add_user_roles

# Or restore from daily backup
./scripts/restore-backup.sh 20240120
```

#### 3. **Schema Verification**

```bash
# Compare production schema with development
bun run scripts/schema-diff.ts

# Verify schema hash
cat migrations/.schema-hash
```
